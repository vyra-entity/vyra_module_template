                ...
                for f in files:
                    if f.endswith(f".{dir_name}"):
                        ...
                        if not _ros2_name_re.match(stem):
                            print(f"⚠️  Skipping '{f}': not a valid ROS2 interface name (contains underscore or starts with lowercase).")
                            continue
...
def build_proto_interfaces(interface_package_path: Path, module_name: str):
    ...
    try:
        ...
        if not proto_files:
            logging.info(f"ℹ️  No .proto files found in {interface_package_path} - skipping proto generation")
            return
